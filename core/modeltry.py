...
class CaptionGenerator(object):
    def __init__(self, dim_feature=[464, 512], dim_embed=512, dim_hidden=1024, n_time_step=16, 
        ...
        self.weight_initializer = slim.l2_regularizer(weight_decay)
        ...
        self.bbox = tf.placeholder(tf.int32, [batch_size, 4])
    ...
    def build_model(self):
        ...
        bbox = tf.to_float(bbox) 
        # print  bbox[:10]
        # mask = tf.to_float(tf.not_equal(bbox, self._null))
        ...
        with tf.variable_scope('model') as scope:   
            ...
            with slim.arg_scope([slim.batch_norm]):
              with slim.arg_scope([slim.conv2d],weights_regularizer=slim.l2_regularizer(weight_decay),
                ...
                h_conv1 = slim.conv2d(x_image, 32, [3, 3], scope='conv1')
                h_conv2 = slim.conv2d(h_conv1, 64, [3, 3], scope='conv2')
                h_pool1 = slim.max_pool2d(h_conv2, [2, 2], scope='pool1')

                h_conv3 = slim.conv2d(h_pool1, 128, [3, 3], scope='conv3')
                h_conv4 = slim.conv2d(h_conv3, 128, [2, 2], scope='conv4')
                # h_pool2 = slim.max_pool2d(h_conv4, [2, 2], scope='pool2')
                h_pool3 = slim.max_pool2d(h_conv4, [2, 2], scope='pool2')
        ...
        # resh = tf.reshape(h_pool3,[-1,h_pool3.get_shape().as_list()[1]**2,128])
        ...
        with tf.variable_scope('decoder') as scope:
                ...
                # deconv2 = tf.nn.relu(slim.conv2d(h_pool3, 128, [3, 3], scope='deconv2')) #print deconv2.get_shape()

                dectran2 = slim.conv2d_transpose(h_pool3, 128, [3,3], [2,2] , scope = 'deconvtran2')#print dectran2.get_shape()
        ...
        loss = 0.0* tf.sqrt(tf.reduce_mean(tf.square(tf.subtract(resh1, resh2))))
        # alpha_list = []
        ...
        # net = mapRnnLayer(h_pool3, 128,scope='net')
        # print net.get_shape()
        # pred = tf.nn.relu(slim.conv2d(h_pool3, 64, [3, 3], scope='predconv'))
        # print pred.get_shape()
        # print pred.get_shape()
        # pred = tf.slice(pred, [ 0, 15, 15 , 0] , [-1, 1, 1, 4] )
        # pred = tf.reshape(pred,[-1,4])
        # pred = 
        pred = slim.fully_connected(slim.flatten(h_pool3), 512, scope = 'fcpred1')
        pred = slim.fully_connected(pred, 128, scope = 'fcpred2')
        pred = slim.fully_connected(pred, 4, scope = 'fcpred3',activation_fn=None,)
        # pred = pred[:, pred.get_shape().as_list()[1]-1, pred.get_shape().as_list()[1]-1, 4]
        # print pred.get_shape()
        # act = tf.reshape(bbox,[-1,4])
        # print act.get_shape()
        print bbox.get_shape()
        loss +=  tf.reduce_sum(tf.reduce_mean(tf.square(tf.subtract(pred, bbox))))
    ...
    def build_sampler(self, max_len=20):
        ...
        # pred = tf.slice(pred, [ 0, 15, 15 , 0] , [-1, 1, 1, 4] )
        # pred = tf.reshape(pred,[-1,4])
        pred = tf.reshape(h_pool3,[-1,h_pool3.get_shape().as_list()[1]**2 * 128])
        pred = slim.fully_connected(pred, 512, scope = 'fcpred1')
        pred = slim.fully_connected(pred, 128, scope = 'fcpred2')
        pred = slim.fully_connected(pred, 4, scope = 'fcpred3') 

        return pred
        # pred = pred[:, pred.get_shape().as_list()[1]-1, pred.get_shape().as_list()[1]-1, 4]
        # loss += tf.reduce_sum(tf.reduce_mean(tf.square(tf.subtract(pred, bbox[:, ])* mask[:, ])))
