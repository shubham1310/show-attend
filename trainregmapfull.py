...
from core.modelregmap1 import CaptionGenerator
...
def main():
    ...
    solver = CaptioningSolver(model, bbox=np.array(bboxes), path=path, n_time_step=1,  n_epochs=epoch, batch_size=batch_size, update_rule=str(update_rule),
                        ...
                        model_path='model/'+ name + 'lstm'+ 'regmap' +update_rule +str(dim_feature1)+'_' + str(batch_size) +'/',
                        test_model='model/'+ name + 'lstm'+ 'regmap' + update_rule +str(dim_feature1)+'_'+ str(batch_size)  +'/model-10',
                        print_bleu=True, log_path='log/'+ name + 'regmap' + update_rule  +str(dim_feature1)+'_' + str(batch_size)  +'/')
