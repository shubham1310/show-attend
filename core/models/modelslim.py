...
class CaptionGenerator(object):
    ...
    #             h_logits += slim.fully_connected(context, self.M, scope = 'decodefc2')
    ...
    def _decode_lstm(self, x, h, context, dropout=False, reuse=False):
        with tf.variable_scope('logits', reuse=reuse):
            ...
            if self.ctx2out:
                # w_ctx2out = tf.get_variable('w_ctx2out', [self.D, self.M], initializer=self.weight_initializer)
                h_logits += slim.fully_connected(context, self.M, normalizer_fn=lambda x: x, activation_fn=None, scope = 'decodefc2')
                # h_logits += tf.matmul(, w_ctx2out)
    ...
    def build_sampler(self, max_len=20):
        ...
        return tf.reshape(alphas, [-1, max_len, 16,16]) , betas, sampled_captions
