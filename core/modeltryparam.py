...
class CaptionGenerator(object):
    ...
    def build_model(self):
        ...
        loss = 0.0001*tf.sqrt(tf.reduce_mean(tf.square(tf.subtract(resh1, resh2))))
        # alpha_list = []
        ...
        pred = tf.reshape(pred,[-1,pred.get_shape().as_list()[1]**2 * 64])
        pred = slim.fully_connected(pred, 512, scope = 'fcpred1',activation_fn=None)
        pred = slim.fully_connected(pred, 128, scope = 'fcpred2',activation_fn=None)
        ...
        loss +=  tf.reduce_sum(tf.reduce_mean(tf.square(tf.subtract(pred, bbox_out[:, ]))))
