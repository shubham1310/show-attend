...
class CaptionGenerator(object):
    def __init__(self, dim_feature=[464, 512], dim_embed=512, dim_hidden=1024, n_time_step=16, 
        ...
        self.features = tf.placeholder(tf.float32, [batch_size, imshape, imshape, channels], name='features')
        self.bbox = tf.placeholder(tf.int32, [batch_size, self.T + 1, 4], name='bboxes')
    ...
    def build_model(self):
        ...
        act = tf.reshape(bbox_out,[-1,4])
        loss += tf.reduce_sum(tf.reduce_mean(tf.square(tf.subtract(pred, act))))
