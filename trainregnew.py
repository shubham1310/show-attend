from core.solvers.solverreg import CaptioningSolver
from core.models.modelregnew import CaptionGenerator
...
def main():
    ...
    model = CaptionGenerator( dim_feature=[dim_feature1, 512], dim_embed=512, batch_size=batch_size,
    ...
    solver = CaptioningSolver(model, bbox=np.array(bboxes), path=path, n_epochs=epoch, batch_size=batch_size, update_rule=str(update_rule),
                        ...
                        model_path='model/lstm'+ 'regtanh' + update_rule +str(dim_feature1)+'_' + str(batch_size) +'/',
                        test_model='model/lstm'+ 'regtanh' + update_rule +str(dim_feature1)+'_'+ str(batch_size)  +'/model-10',
                        print_bleu=True, log_path='log/'+ 'regtanh'+ update_rule  +str(dim_feature1)+'_' + str(batch_size)  +'/')
