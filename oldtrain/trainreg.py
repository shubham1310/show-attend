from core.solverreg import CaptioningSolver
from core.modelreg import CaptionGenerator
...
def main():
    ...
    solver = CaptioningSolver(model, bbox=np.array(bboxes), path=path, n_epochs=epoch, batch_size=batch_size, update_rule=str(update_rule),
                        ...
                        model_path='model/lstm'+ 'reg' + update_rule +str(dim_feature1)+'_' + str(batch_size) +'/',
                        test_model='model/lstm'+ 'reg' + update_rule +str(dim_feature1)+'_'+ str(batch_size)  +'/model-10',
                        print_bleu=True, log_path='log/'+ 'reg'+ update_rule  +str(dim_feature1)+'_' + str(batch_size)  +'/')
