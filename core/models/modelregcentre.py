...
class CaptionGenerator(object):
    def __init__(self, dim_feature=[464, 512], dim_embed=512, dim_hidden=1024, n_time_step=16, 
        ...
        self.bbox = tf.placeholder(tf.int32, [None, self.T + 1, 2])
    ...
    def _decode_lstm(self, x, h, context, dropout=False, reuse=False):
        with tf.variable_scope('logits', reuse=reuse):
            ...
            w_last = tf.get_variable('w_last', [self.V, 2], initializer=self.weight_initializer)
            b_last = tf.get_variable('b_last', [2], initializer=self.const_initializer)
            ...
            last_logits = tf.matmul(out_logits, w_last) + b_last
            return last_logits
    ...
    def build_model(self):
        ...
        if self.alpha_c > 0:
            ...
            alpha_reg = self.alpha_c * tf.reduce_sum((6./self.L - alphas_all) ** 2)     
            loss += alpha_reg
    ...
    def build_sampler(self, max_len=20):
        ...
        for t in range(max_len):
            if t == 0:
                ...
                x = tf.fill([tf.shape(features)[0], 2], tf.to_float(0))
            else:
                x = sampled_word
