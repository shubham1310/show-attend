...
class CaptioningSolver(object):
    def __init__(self, model, path, caption, **kwargs):
        """
        Required Arguments:
    ...
    def train(self,tp="", chunk=0):
        # train/val dataset


        features = np.load(self.path + 'trainRandomTRs.npy', mmap_mode='r')
        n= self.captions.shape[0]
        ...
        alphas, _, generated_captions = self.model.build_sampler(max_len=18)
        ...
        with tf.Session(config=config) as sess:
            ...
            for e in range(self.n_epochs):
                ...
                for i in range(n_iters_per_epoch):
                    ...
                    # captions_batch = captions[image_idxs_batch]
                ...
                if self.print_bleu and (e % 5 ==0):
                    all_gen_cap = np.ndarray((n_iters_val*self.batch_size, 18))
                    alp=[]
                    for i in range(n_iters_val):
                        ...
                        alpha, gen_cap = sess.run(
                            [alphas, generated_captions], feed_dict=feed_dict)
                        all_gen_cap[i *self.batch_size:(i + 1) * self.batch_size] = gen_cap
                        for b in alpha:
                            alp.append(b)

                    # for i in all_gen_cap:
                    #     print i
                    ...
                    for l in range(5):
                        ...
                        while j >= len(all_decoded):
                            j=j/10
                        print "\nGround:",
                        for k in range(1,17):
                            # if self.captions[n_examples + j][k] == 10:
                            #     continue
                            if not(self.captions[n_examples + j][k] == 11):
                                print self.captions[n_examples + j][k],
                        print '\n',
                        print "Genera: %s" % all_decoded[j]

                        for k in range(15):
                            # print alp[0].shape
                            img = (((alp[j][k] - alp[j][k].min()) / (alp[j][k].max() - alp[j][k].min())) * 255.9).astype(np.uint8)
                            img=Image.fromarray(alp[j][k], 'L')
                            if not os.path.exists('imgs/'+tp+'/alp' + str(e) +'/' + str(j)):
                                os.makedirs('imgs/'+tp+'/alp' + str(e) +'/' + str(j))
                            img.save('imgs/'+tp+'/alp' + str(e) +'/' + str(j) +'/' + str(k) +'.jpg')
    ...
    def test(self, data, split='test', attention_visualization=True, save_sampled_captions=True):
        ...
        alphas, betas, sampled_captions = self.model.build_sampler(
            max_len=18)    # (N, max_len, L), (N, max_len)
